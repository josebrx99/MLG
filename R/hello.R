supuestos = function(model) {
  ...
  plot(model$fitted.values, studres(model), ylab='residuos estudentizados',
       xlab='valores ajustados', main='valores ajustados vs residuos estudentizados', pch = 19, col = "indianred1")
  abline(h=0,lty=2)
  lines(lowess(studres(model) ~ model$fitted.values), col = "black")


  plot(model$fitted.values, abs(studres(model)), ylab='valor absoluto residuos estudentizados',
       xlab='valores ajustados',main='valores ajustados vs valor absoluto residuos estudentizados', pch = 19, col = "indianred1")
  abline(h=0,lty=2)
